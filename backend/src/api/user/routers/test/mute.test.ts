...
xdescribe("User Router: User Relation Mute Actions", () => {
  ...
  describe("POST /:id/mute", () => {
    ...
    it("should return 200 and the updated users data when mute is added", async () => {
    ...
    it("should return 400 if the provided ID is not a valid MongoDB ID", async () => {
      ...
      const res = await request(app).post(`/${invalidUserId}/mute`).set("Cookie", [token]);
    ...
    it("should return 500 if the user with the given ID is not found", async () => {
      ...
      const res = await request(app).post(`/${id}/mute`).set("Cookie", [token]);
  ...
  describe("POST /:userId/mute/:postId/fromPost", () => {
    ...
    it("should return 400 if the provided userId or postId is not a valid MongoDB ID", async () => {
      ...
      const res = await request(app)
        .post(`/${invalidUserId}/mute/${postId}/fromPost`)
        .set("Cookie", [token]);
      expect(res.statusCode).toEqual(400);
      ...
      const res_2 = await request(app)
        .post(`/${userId}/mute/${invalidPostId}/fromPost`)
        .set("Cookie", [token]);
    ...
    it("should return 404 if the post with the given ID is not found", async () => {
      ...
      const res = await request(app)
        .post(`/${userId}/mute/${nonExistentPostId}/fromPost`)
        .set("Cookie", [token]);
    ...
    it("should return 404 if the Follower or Following with the given ID is not found", async () => {
      ...
      const res = await request(app)
        .post(`/${followerId}/mute/${nonExistentPostId}/fromPost`)
        .set("Cookie", [token]);
      expect(res.statusCode).toEqual(404);
    ...
    it("should return 500 if an internal server error occurs", async () => {
      ...
      const res = await request(app)
        .post(`/${userId}/mute/${postId}/fromPost`)
        .set("Cookie", [token]);
  ...
  describe("DELETE /:id/mute", () => {
    ...
    it("should return 401 if the user is not logged in", async () => {
      const res = await request(app).delete(`/${validUser.id}/mute`);
    ...
    it("should return 400 if the provided ID is not a valid MongoDB ID", async () => {
      const invalidUserId = "12345";
      const res = await request(app).delete(`/${invalidUserId}/mute`).set("Cookie", [token]);
      expect(res.statusCode).toEqual(400);
      expect(res.body.message).toContain("Invalid user id");
    });
    ...
    it("should return 404 if the user with the given ID is not found", async () => {
      const id = new Types.ObjectId();
      await UserModel.findByIdAndDelete(id);
      const res = await request(app).delete(`/${id}/mute`).set("Cookie", [token]);
      expect(res.statusCode).toEqual(404);
      expect(res.body.message).toContain("You have not muted this User");
    });
