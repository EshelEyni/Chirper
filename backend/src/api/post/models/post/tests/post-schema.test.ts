...
describe("PostModel", () => {
  ...
  describe("Schema", () => {
    ...
    describe("Images", () => {
      ...
      it("Should limit image array to 4 elements.", async () => {
        const body = {
          imgs: [{ url: "https://www.google.com", sortOrder: undefined }],
        };

        await expect(createTestPost({ body })).rejects.toThrow("Image sort order is required");
      });
    });
    ...
    fdescribe("Repliers", () => {
      it("Should validate repliedPostDetails with valid postId and postOwner.", async () => {
        ...
        const body = { repliedPostDetails: [createRepliedPostDetails(repliedPost)] };
        await expect(createTestPost({ body })).resolves.toBeDefined();
      });
      ...
      it("Should invalidate repliedPostDetails when postOwner.username is missing.", async () => {
        const post = await createTestPost({});
        const body = {
          repliedPostDetails: [
            {
        ...
        await expect(createTestPost({ body })).rejects.toThrow(
          "Replied post owner must have a username"
        );
      });
    });
