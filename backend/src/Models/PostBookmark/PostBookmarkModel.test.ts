...
describe("Bookmark Post Model", () => {
  ...
  describe("Schema validation", () => {
    it("should require postId", async () => {
      const newBookmark = new PostBookmarkModel({ bookmarkOwnerId });
    ...
    it("should require bookmarkOwnerId", async () => {
      ...
      await expect(newBookmark.save()).rejects.toThrow(
        "PostBookmarkModel validation failed: bookmarkOwnerId: Path `bookmarkOwnerId` is required."
      );
    });
    ...
    it("should check if the referenced user exists", async () => {
      ...
      await expect(newBookmark.save()).rejects.toThrow(
        "PostBookmarkModel validation failed: bookmarkOwnerId: Referenced user does not exist"
      );
    });

    it("should create timestamps", async () => {
      await deleteAndCreateMocks();
      const newBookmark = await PostBookmarkModel.create({ postId, bookmarkOwnerId });
  ...
  describe("Indexes", () => {
    it("should enforce unique constraint on postId and bookmarkOwnerId", async () => {
      await deleteAndCreateMocks();
      const newBookmark1 = new PostBookmarkModel({ postId, bookmarkOwnerId });
      await newBookmark1.save();

      const newBookmark2 = new PostBookmarkModel({ postId, bookmarkOwnerId });
  ...
  describe("Post-save hooks", () => {
    it("should populate the post field with post.isBookmarked is true", async () => {
      await deleteAndCreateMocks();
      const newBookmark = new PostBookmarkModel({ postId, bookmarkOwnerId });
